  ...
  const category = searchParam.get('category') || 'all';
    ...
    return (
        <Container>
            ...
            <Row>
            {products.length>0 && (category==='all'? products.map((filteredProduct, index)=> (
                <Col md={3} className="mb-3" key={index}>
                    <SingleProduct product={filteredProduct}/> 
                    </Col>
                    )):products.filter(product => product.category?.[0].uid == category).map((filteredProduct, index)=> (
                        <Col sm={6} lg={4} className="mb-3" key={index}>
                        <SingleProduct product={filteredProduct}/>
                        </Col>
